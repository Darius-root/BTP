...
export const createColumns = (onDelete) => [
    ...
    },
    {
        ...
        cell: ({ row }) => h('div', {}, [
            ...
            h('div', { class: 'text-xs text-gray-500 dark:text-gray-400 mt-0.5' }, 'Arrondissement')
        ]),
    },
    {
        accessorKey: 'commune.libelle',
        header: 'Commune',
        cell: ({ row }) => {
            const commune = row.original.commune?.libelle || '-'
            return h('div', { class: 'text-sm text-gray-600 dark:text-gray-400' }, commune)
        },
    },
    {
    ...
    },
    {
        ...
        cell: ({ row }) => {
            const arrondissement = row.original

            return h('div', { class: 'flex justify-end gap-2' }, [
                // Voir
                h(Link, {
                    href: route('arrondissements.show', arrondissement.id),
                ...
                h(Button, {
                    variant: 'ghost',
                    size: 'sm',
                    onClick: () => onDelete(arrondissement),
