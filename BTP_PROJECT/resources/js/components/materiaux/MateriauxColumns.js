...
export const createMateriauxColumns = (onDelete) => [
    {
        accessorKey: 'code',
        header: ({ column }) =>
            h(Button, {
    ...
    {
        accessorKey: 'unite.libelle',
        header: 'Unité de mesure',
        cell: ({ row }) => {
            const unite = row.original.unite?.libelle || '-'
            return h('div', { class: 'text-sm text-gray-600 dark:text-gray-400' }, unite)
        },
    },
    {
    ...
    },
    {
        ...
        cell: ({ row }) => {
            const materiau = row.original

            return h('div', { class: 'flex justify-end gap-2' }, [
                // Voir
                h(Link, {
                    href: route('materiaux.show', materiau.id),
