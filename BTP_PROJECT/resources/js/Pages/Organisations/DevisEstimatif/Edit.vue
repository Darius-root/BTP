...
const props = defineProps<{
    ...
    niveauxDisponibles: Niveau[];
    unites: Unite[];
}>();

// Initialiser le formulaire avec les données du devis existant
// Initialiser le formulaire avec les données du devis existant
const form = useForm({
    intitule: props.devis.intitule,
    batiment_id: props.batiment.id,
    niveaux: props.devis.composants.map((n) => ({
        niveau_id: n.niveau_id,
        composants: (n.composants ?? []).map((c) => ({ ...c })), // <-- fallback [] si undefined
    })),
});
...
const addNiveau = () => {
    form.niveaux.push({
        niveau_id: null,
        composants: [
            { code: "", piece: "", unite_id: null, qte: 0, prix_unitaire: 0 },
        ],
    });
};
...
// Niveaux disponibles (éviter doublons)
const availableNiveaux = (currentIndex: number) => {
    const selectedIds = form.niveaux
        .filter((_, i) => i !== currentIndex)
        .map((n) => n.niveau_id)
        .filter((id): id is number => id !== null);
    return props.niveauxDisponibles.filter((n) => !selectedIds.includes(n.id));
};

// Sous-totaux
const niveauSubtotal = (niveau: NiveauForm) => {
    return niveau.composants.reduce(
        (sum, comp) => sum + comp.qte * comp.prix_unitaire,
        0
    );
};
...
const totalGeneral = computed(() =>
    form.niveaux.reduce((sum, n) => sum + niveauSubtotal(n), 0)
);

// Soumettre le formulaire
const submit = () => {
    form.put(
        route("batiments.devisestimatif.update", {
...
<template>
    <Head :title="`Modifier le Devis Estimatif ${batiment.nom}`" />
    <SidebarProvider>
        <AdminLayout>
            <PageBreadcrumb
                :pageTitle="`Modifier Devis Estimatif - ${batiment.nom}`"
            />
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle class="flex items-center gap-2">
                                ...
                                Modifier un Devis Estimatif pour le bâtiment :
                            </CardTitle>
                        </CardHeader>
                        <CardContent class="space-y-4">
                            ...
                            <div class="mt-4 flex justify-end">
                                ...
                                >
                                    <CheckCircle class="w-4 h-4 mr-1" /> Mettre
                                    à jour le Devis
                                </Button>
                            </div>
                        </CardContent>
