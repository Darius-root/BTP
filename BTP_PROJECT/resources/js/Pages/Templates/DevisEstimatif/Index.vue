...
import { Plus, Shield, Eye, Copy } from 'lucide-vue-next'
...
const filteredTemplates = computed(() => {
    if (!showSystemOnly.value) return props.templates
...
function viewFullTemplate(template) {
    if (template) router.visit(route('templates.show', template.id))
}

function reuseTemplate(template) {
    if (template) router.visit(route('templates.reuse', template.id))
}
...
<template>
    ...
    <SidebarProvider>
        <AdminLayout>
            ...
            <div class="space-y-6">
                <Card class="border-0 shadow-none rounded-2xl p-5 lg:p-6">
                    <CardHeader class="px-0 pt-0">
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                            <div>
                                <CardTitle class="text-3xl">Templates de Devis Estimatif</CardTitle>
                                <CardDescription class="mt-1">
                                    Liste des devis templates de Devis Estimatif réutilisables
                                </CardDescription>
                            </div>
                        </div>
                    </CardHeader>
                ...
                <DialogContent class="max-w-3xl max-h-[80vh] overflow-y-auto">
                    ...
                    <div v-if="selectedTemplate" class="space-y-4 mt-4">
                        <div class="grid grid-cols-2 gap-4 p-4 bg-gray-50 dark:bg-gray-900 rounded-lg">
                            ...
                            <div>
                                <p class="text-sm text-gray-500">Composants</p>
                    ...
                    <DialogFooter class="mt-6 flex flex-wrap gap-2">
                        <Button variant="outline" @click="detailsOpen = false">Fermer</Button>

                        <button
                            v-if="selectedTemplate.canView"
                            @click="viewFullTemplate(selectedTemplate)"
                        ...
                        <button
                            v-if="selectedTemplate.canCreate"
                            @click="reuseTemplate(selectedTemplate)"
