                            ...
                            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                                <div>
                                    ...
                                    <CardDescription class="mt-1">
                                        Gérez les matériaux de votre système
                                    </CardDescription>
                                </div>
                                <Link
                                    :href="route('materiaux.create')"
                        ...
                        <CardContent class="px-0">
                            <div class="space-y-6">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <div class="relative mt-1">
                                            ...
                                            <Input
                                                id="searchFilter"
                                                type="text"
                                                placeholder="Rechercher par code, nom ou unité..."
                                ...
                                <div class="text-sm text-gray-600 dark:text-gray-400">
                                    {{ filteredData.length }} matériau(x) affiché(s)
                                    <template v-if="filteredData.length !== materiaux.length">
                                        sur {{ materiaux.length }}
                                    </template>
                                </div>
                                    ...
                                    <Table>
                                        ...
                                        <TableBody>
                                            <template v-if="filteredData.length">
                                                <TableRow
                                                    v-for="materiau in filteredData"
                                                    :key="materiau.id"
                                                    class="hover:bg-gray-50 dark:hover:bg-gray-800/50"
                                                >
                                                    <TableCell class="font-medium">
                                                        <div class="flex items-center gap-2">
                                                            <div class="w-8 h-8 flex items-center justify-center rounded-lg bg-green-100 dark:bg-green-900 text-green-600 dark:text-green-400">
                                                    ...
                                                    <TableCell class="text-right">
                                                        <div class="flex justify-end gap-2">
                                                            <Link
                                                                :href="route('materiaux.edit', materiau.id)"
                                                    ...
                                                    <TableCell colspan="4" class="h-24 text-center">
                                                        <div class="text-center py-8 text-gray-500">
                                                            ...
                                                            <p class="text-sm mt-1">
                                                                {{ globalFilter ? 'Aucun résultat pour votre recherche' : 'Commencez par créer votre premier matériau' }}
                                                            </p>
                                                        </div>
                                                    </TableCell>
                                                </TableRow>
...
import { Plus, Edit, Trash2, Package, Search, X } from 'lucide-vue-next'
...
const filteredData = computed(() => {
    if (!globalFilter.value) return props.materiaux

    const searchTerm = globalFilter.value.toLowerCase()
    return props.materiaux.filter(materiau =>
        materiau.code.toLowerCase().includes(searchTerm) ||
        materiau.nom.toLowerCase().includes(searchTerm) ||
        materiau.unite.libelle.toLowerCase().includes(searchTerm)
    )
})
...
const confirmDelete = (materiau) => {
    if (confirm(`Êtes-vous sûr de vouloir supprimer le matériau "${materiau.nom}" ? Cette action est irréversible.`)) {
