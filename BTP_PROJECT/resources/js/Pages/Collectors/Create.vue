...
<template>
    ...
    <Head title="Ajouter un collecteur" />

    <SidebarProvider>
        <AdminLayout>
            <PageBreadcrumb pageTitle="Ajouter un collecteur" />

            <div class="mx-auto max-w-xl">
                <Card>
                    <CardHeader>
                        <CardTitle>Ajouter un collecteur à l’organisation active</CardTitle>
                    </CardHeader>
                    ...
                    <CardContent class="space-y-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <Input id="email" v-model="form.email" type="email"
                                placeholder="ex: collecteur@email.com" />
                        ...
                        <div class="space-y-2">
                                    ...
                                    <Command>
                                        ...
                                        <CommandList>
                                            ...
                                            <CommandGroup heading="Rôles disponibles">
                                                <CommandItem v-for="role in roles" :key="role.id" :value="role.id"
                                                    @select="toggleRole(role.id)">
                                                    <Check class="mr-2 h-4 w-4" :class="form.roles.includes(role.id)
                                                        ? 'opacity-100'
                                                        : 'opacity-0'" />
                                                    {{ role.name }}
                                                </CommandItem>
                            ...
                            <div class="flex flex-wrap gap-2 pt-2">
                                <Badge v-for="roleId in form.roles" :key="roleId" variant="secondary">
                        ...
                        <div class="flex justify-end gap-2 pt-4">
                            <Button variant="outline" @click="router.visit(route('collectors.index'))">
                                Annuler
                            </Button>

                            <Button :disabled="form.processing || !form.email || form.roles.length === 0"
                                @click="submit">
