...
const currentPageTitle = ref('Détails de l’arrondissement')
...
<template>
    ...
    <Head title="Détails de l’arrondissement" />
    ...
    <SidebarProvider>
        <AdminLayout>
                    ...
                    <Card class="border-0 shadow-none">
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                                <div>
                                    ...
                                    <CardDescription class="mt-1">
                                        Informations détaillées de l’arrondissement
                                    </CardDescription>
                                </div>
                        ...
                        <CardContent class="px-0">
                            <div class="space-y-4 text-gray-700">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        ...
                                        <p>{{ arrondissement.libelle }}</p>
                                    </div>

                                    <div>
                                        <Label class="font-semibold">Code :</Label>
                                        <p>{{ arrondissement.code }}</p>
                                    </div>

                                    <div>
                                        <Label class="font-semibold">Commune :</Label>
                                        <p>{{ arrondissement.commune?.libelle ?? 'Pas encore lié à une commune' }}</p>
                                    </div>

                                    <div>
                                        <Label class="font-semibold">Date de création :</Label>
                                        <p>{{ formatDate(arrondissement.created_at) }}</p>
                                    </div>

                                    <div class="md:col-span-2">
                                        <Label class="font-semibold">Dernière modification :</Label>
                                        <p>{{ formatDate(arrondissement.updated_at) }}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="flex flex-wrap items-center justify-end gap-3 mt-6">
                                <Link :href="route('arrondissements.edit', arrondissement.id)"
            ...
            <!-- Dialog suppression -->
            <DeleteDialog :open="deleteOpen" :item="arrondissement" resource="arrondissements"
                :label="`l'arrondissement ${arrondissement.libelle}`" @update:open="deleteOpen = $event" />
